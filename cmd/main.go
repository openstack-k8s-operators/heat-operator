...
// Package main is the entry point for the heat-operator.
package main
...
import (
	...
	"github.com/openstack-k8s-operators/heat-operator/internal/controller"
	webhookv1beta1 "github.com/openstack-k8s-operators/heat-operator/internal/webhook/v1beta1"
...
func main() {
	...
	if err := (&controller.HeatAPIReconciler{
		Client:  mgr.GetClient(),
		Scheme:  mgr.GetScheme(),
		Kclient: kclient,
	}).SetupWithManager(context.Background(), mgr); err != nil {
	...
	if err := (&controller.HeatCfnAPIReconciler{
		Client:  mgr.GetClient(),
		Scheme:  mgr.GetScheme(),
		Kclient: kclient,
	}).SetupWithManager(context.Background(), mgr); err != nil {
	...
	if err := (&controller.HeatEngineReconciler{
		Client:  mgr.GetClient(),
		Scheme:  mgr.GetScheme(),
		Kclient: kclient,
	}).SetupWithManager(context.Background(), mgr); err != nil {
	...
	if os.Getenv("ENABLE_WEBHOOKS") != "false" {
		if err := webhookv1beta1.SetupHeatWebhookWithManager(mgr); err != nil {
