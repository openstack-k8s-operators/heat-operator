...
func (r *HeatCfnAPIReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager) error {
	...
	// index topologyField
	if err := mgr.GetFieldIndexer().IndexField(context.Background(), &heatv1beta1.HeatCfnAPI{}, topologyField, func(rawObj client.Object) []string {
	...
	acSecretFn := func(_ context.Context, o client.Object) []reconcile.Request {
		...
		if name == expectedSecretName {
			...
			if err := r.List(context.Background(), heatCfnAPIs, listOpts...); err != nil {
				return nil
			}

			// Enqueue reconcile for all heat CFN API instances
			for _, cr := range heatCfnAPIs.Items {
...
func (r *HeatCfnAPIReconciler) reconcileNormal(ctx context.Context, instance *heatv1beta1.HeatCfnAPI, helper *helper.Helper) (ctrl.Result, error) {
	...
	deplSpec, err := heatcfnapi.Deployment(instance, inputHash, serviceLabels, topology, memcached)
	if err != nil {
		instance.Status.Conditions.Set(condition.FalseCondition(
