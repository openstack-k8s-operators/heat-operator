...
var _ = Describe("Heat controller", func() {
	...
	When("An ApplicationCredential is created for Heat", func() {
		...
		It("should render ApplicationCredential auth in Heat config", func() {
			Eventually(func(g Gomega) {
				api := GetHeat(heatName)
				g.Expect(api).NotTo(BeNil())
				g.Expect(api.Spec.Auth.ApplicationCredentialSecret).To(Equal(acSecretName))
			}, timeout, interval).Should(Succeed())

			// Check CinderAPI config secret for AC auth
			Eventually(func(g Gomega) {
				cfgSecret := th.GetSecret(heatConfigSecretName)
				...
				g.Expect(conf).NotTo(ContainSubstring("username = heat"))
