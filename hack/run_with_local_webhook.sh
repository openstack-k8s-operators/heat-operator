...
- admissionReviewVersions:
  ...
  clientConfig:
    caBundle: ${CA_BUNDLE}
    url: https://${CRC_IP}:${WEBHOOK_PORT}/validate-heat-openstack-org-v1beta1-heat
  failurePolicy: Fail
...
- admissionReviewVersions:
  ...
  clientConfig:
    caBundle: ${CA_BUNDLE}
    url: https://${CRC_IP}:${WEBHOOK_PORT}/mutate-heat-openstack-org-v1beta1-heat
  failurePolicy: Fail
  matchPolicy: Equivalent
  name: mheat.kb.io
  objectSelector: {}
...
if [ -n "${CSV_NAME}" ]; then
    ...
    if CR_NAME=$(oc get openstack -n openstack-operators -o name); then
        ...
        To restore it, use:
        oc patch ${CR_NAME} -n openstack-operators --type=merge -p '{\"spec\": {\"operatorOverrides\": [{\"name\": \"heat\", \"replicas\": 1}]}}'\n"

        oc patch ${CR_NAME} -n openstack-operators --type=merge -p '{"spec": {"operatorOverrides": [{"name": "heat", "replicas": 0}]}}'
    fi
fi
